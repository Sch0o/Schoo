#include"schoo/function/render/texture.hpp"
...
namespace schoo {
    ...
    Texture::Texture(void *buffer,vk::DeviceSize size, uint32_t texWidth, uint32_t texHeight) {
        auto &device = Context::GetInstance().device;
        std::unique_ptr<Buffer> stagingBuffer(new Buffer(size, vk::BufferUsageFlagBits::eTransferSrc,
                                                         vk::MemoryPropertyFlagBits::eHostCoherent |
                                                         vk::MemoryPropertyFlagBits::eHostVisible));
        ...
        transitionImageLayout(vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
        copyBufferToImage(stagingBuffer->buffer, texWidth, texHeight);
        transitionImageLayout(vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
